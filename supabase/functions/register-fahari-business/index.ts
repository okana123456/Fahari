...
serve(async (req) => {
  ...
  try {
    ...
    const expectedKey = String(Deno.env.get("FAHARI_REGISTRATION_KEY") || "").trim();
    ...
    if (!await secureEqual(suppliedKey, expectedKey)) {
      return json({ ok: false, message: "Invalid business registration key. Contact the Fahari platform owner." }, 403);
    }

    const supabaseUrl = Deno.env.get("FAHARI_PROJECT_URL") || "";
    const anonKey = Deno.env.get("FAHARI_ANON_KEY") || "";
    const serviceRoleKey = Deno.env.get("FAHARI_SERVICE_ROLE_KEY") || "";
