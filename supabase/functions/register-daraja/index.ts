...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const supabaseUrl = Deno.env.get("FAHARI_PROJECT_URL") || "";
    const serviceKey = Deno.env.get("FAHARI_SERVICE_ROLE_KEY") || "";
    if (!supabaseUrl || !serviceKey) {
      return json({
        success: false,
        error: "Fahari Supabase secrets are missing. Add FAHARI_PROJECT_URL and FAHARI_SERVICE_ROLE_KEY.",
      }, 400);
    }
    ...
    if (!staffResult.ok) {
      return json({
        success: false,
        error: `Could not verify the Fahari administrator: ${String(staffResult.data?.message || staffResult.status)}`,
      }, 500);
    }
    ...
    const credentialsResult = await backendRequest(
      supabaseUrl,
      serviceKey,
      `/rest/v1/fahari_daraja_credentials?select=consumer_key,consumer_secret&business_id=eq.${encodeURIComponent(String(staff.business_id))}&limit=1`,
    );
    const savedCredentials = Array.isArray(credentialsResult.data) ? credentialsResult.data[0] : null;
    if (!credentialsResult.ok) {
      return json({
        success: false,
        error: "The secure Daraja credential store is not ready. Run fahari-daraja-secure-credentials.sql in Supabase, then redeploy this function.",
      }, 500);
    }

    const cleanKey = suppliedKey || String(savedCredentials?.consumer_key || Deno.env.get("FAHARI_DARAJA_CONSUMER_KEY") || "").trim();
    const cleanSecret = suppliedSecret || String(savedCredentials?.consumer_secret || Deno.env.get("FAHARI_DARAJA_CONSUMER_SECRET") || "").trim();
    ...
    const validationUrl = `${supabaseUrl}/functions/v1/fahari-c2b-validation`;
    const confirmationUrl = `${supabaseUrl}/functions/v1/fahari-payment-callback`;

    // The Fahari production app is provisioned for C2B v2. Do not hide a v2
    ...
    if (suppliedKey && suppliedSecret) {
      const saveCredentialsResult = await backendRequest(
        supabaseUrl,
        serviceKey,
        "/rest/v1/fahari_daraja_credentials?on_conflict=business_id",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!saveCredentialsResult.ok) {
        return json({
          success: false,
          error: "Safaricom accepted the URLs, but Fahari could not save the credentials securely. " + String(saveCredentialsResult.data?.message || saveCredentialsResult.status),
        }, 500);
      }
    }
