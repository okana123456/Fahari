...
serve(async (req) => {
  console.log("Fahari callback request", { method: req.method, url: req.url });
  ...
  try {
    ...
    console.log("Fahari callback payload", {
      transId,
    ...
    if (!transId) {
      console.log("Fahari callback ignored: missing TransID");
      return accepted();
    }
    ...
    if (!shortcode || !amount || amount <= 0) {
      console.log("Fahari callback ignored: missing shortcode or amount", { transId, shortcode, amount });
      return accepted();
    }

    const supabase = createClient(
      Deno.env.get("FAHARI_PROJECT_URL") || "",
      Deno.env.get("FAHARI_SERVICE_ROLE_KEY") || "",
    ...
    if (existingCallback) {
      ...
      console.log("Fahari duplicate callback ignored", { transId });
      return accepted();
    }
    ...
    console.log("Fahari callback business lookup", { transId, shortcode, businessId, autoConfirm: settings?.mpesa_auto_confirm });
    ...
    if (queueError) {
      if (String(queueError.message || "").toLowerCase().includes("duplicate")) {
        console.log("Fahari duplicate callback insert ignored", { transId });
        return accepted();
      }
      throw queueError;
    }
      ...
      if (queue?.id) {
        await supabase
          ...
          .update({
            ...
            processing_message: `No Fahari business matched shortcode ${shortcode}`,
          })
          .eq("id", queue.id);
      }
      console.log("Fahari callback stored without business match", { transId, shortcode });
      return accepted();
    }

    const client = await findClient(supabase, businessId, accountNumber, payerPhone);
    if (!client) {
      console.log("Fahari callback unmatched client", { transId, businessId, accountNumber, payerPhone });
    ...
    if (!loan) {
      ...
      console.log("Fahari callback confirmed charge deposit", { transId, businessId, clientId: client.id, amount });
      return accepted();
    }

    if (!settings?.mpesa_auto_confirm) {
      console.log("Fahari callback queued for manual confirmation", { transId, businessId, loanId: loan.id });
      if (queue?.id) {
        await supabase
          .from("mpesa_callback_queue")
          .update({
    ...
    console.log("Fahari callback confirmed repayment", { transId, businessId, clientId: client.id, loanId: loan.id, repaymentId: repayment.id, appliedAmount, excessAmount, newBalance });
    return accepted();
  } catch (error) {
    console.error("Fahari C2B callback error:", error);
    if (trackedSupabase && trackedQueueId) {
      try {
        ...
        console.error("Fahari callback audit update failed", { trackedTransId, trackingError });
      }
    }
    return accepted();
  }
