...
serve(async (req) => {
  ...
  try {
    ...
    const accountReference = `FAHA${String(staff.business_id).replace(/[^a-z0-9]/gi, "").slice(-7).toUpperCase()}`.slice(0, 12);
    const payload = {
      ...
      TransactionDesc: "Fahari system subscription",
    };
    ...
    if (saveError) {
      return json({
        ok: false,
        message: `M-Pesa prompt was sent, but Fahari could not save the payment request: ${saveError.message}`,
        checkout_request_id: stk.CheckoutRequestID,
      }, 500);
    }
